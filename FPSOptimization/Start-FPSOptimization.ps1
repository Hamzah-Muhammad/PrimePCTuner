# Start-FPSOptimization.ps1 — v0.2 DRY-RUN GUI, Prime Investing theme (PCOptimizationServices suite)
# Opens a branded window, detects PC specs, AUTO-SCANS all catalog items on launch and
# shows each as a checkbox row: green "APPLIED" pill when already done, gold "PENDING"
# when it would change. Uncheck anything, Re-scan any time. NOTHING IS CHANGED in the
# dry-run versions — the apply engine is v2.
#
...
# ---------- Prime Investing brand palette (prime_theme / PrimeStocks App.css) ----------
$P = @{
...
<Window xmlns="http://schemas.microsoft.com/winfx/2006/xaml/presentation"
        ...
        Title="FPSOptimization · Prime Investing"
  ...
  <Grid ClipToBounds="True">
    <!-- brand radial glows: green top-center, gold bottom-right -->
    ...
    <Grid Margin="26,20,26,14">
      <Grid.RowDefinitions>
        <RowDefinition Height="Auto"/>  <!-- topbar -->
        <RowDefinition Height="Auto"/>  <!-- header -->
        <RowDefinition Height="Auto"/>  <!-- specs + stats -->
        <RowDefinition Height="*"/>     <!-- list -->
        <RowDefinition Height="Auto"/>  <!-- controls -->
        <RowDefinition Height="Auto"/>  <!-- brand footer -->
      </Grid.RowDefinitions>
      ...
      <DockPanel Grid.Row="0">
        <StackPanel Orientation="Horizontal" DockPanel.Dock="Left">
          ...
          <TextBlock VerticalAlignment="Center" Margin="10,0,0,0" FontSize="14" FontWeight="Bold">
            <Run Foreground="$($P.Green)">@</Run><Run Foreground="$($P.Text)">humzeeny</Run>
          </TextBlock>
        </StackPanel>
      ...
      <StackPanel Grid.Row="1" Margin="0,18,0,10">
        <TextBlock Text="P C  O P T I M I Z A T I O N  S U I T E   ·   D R Y  R U N" FontSize="11"
                   FontWeight="Bold" Foreground="$($P.Green)"/>
        <TextBlock Margin="0,4,0,0" FontSize="30" FontWeight="ExtraBold" Foreground="$($P.Text)">
          <Run>FPS </Run><Run Foreground="$($P.Green)">Optimization</Run>
        </TextBlock>
        <TextBlock x:Name="SubTitle" Margin="0,4,0,0" FontSize="12.5" Foreground="$($P.Muted)" TextWrapping="Wrap"
                   Text="Scanned automatically against your system — green means already applied. Uncheck anything you don't want. Nothing is changed in dry-run mode."/>
      </StackPanel>
      ...
      <DockPanel Grid.Row="5" Margin="0,10,0,0">
        <TextBlock DockPanel.Dock="Left" FontSize="11.5" Foreground="$($P.Muted)">
          <Run Foreground="$($P.Green)">@</Run><Run>humzeeny</Run><Run>  ·  Prime Investing</Run>
        </TextBlock>
        <TextBlock DockPanel.Dock="Right" HorizontalAlignment="Right" FontSize="11.5"
                   Foreground="$($P.Muted)" Text="FPSOptimization v0.2 · dry run — no changes applied"/>
      </DockPanel>
    </Grid>
  </Grid>
</Window>
...
$ui = @{}
foreach ($name in 'SubTitle','SpecsPanel','StatsPanel','ItemsHost','BtnAll','BtnNone','BtnNoL3','BtnReport','BtnScan','StatusText') {
...
foreach ($k in $specPairs.Keys) {
    ...
    $val = [Windows.Documents.Run]::new([string]$specPairs[$k]); $val.Foreground = $P.Text
...
# ---------- scan-stats chips (populated after each scan) ----------
$StatDefs = @(
...
function Invoke-Scan {
    ...
    foreach ($r in $Rows) {
        ...
        if (-not $r.CheckBox.IsChecked) {
            Set-RowStatus $r 'SKIPPED' 'unchecked by user'
            $counts.skipped++
        ...
        try {
            ...
            switch ($res.Compliant) {
                $true   { Set-RowStatus $r 'APPLIED' $res.Current; $counts.applied++; $st = 'APPLIED' }
                $false  { Set-RowStatus $r 'PENDING' $res.Current; $counts.pending++; $st = 'PENDING' }
                default { Set-RowStatus $r 'REVIEW'  $res.Current; $counts.review++;  $st = 'REVIEW' }
            }
            $results.Add([pscustomobject]@{ Id = $r.Item.Id; Name = $r.Item.Name; Status = $st; Current = $res.Current; Target = $r.Item.Target })
        } catch {
            Set-RowStatus $r 'ERROR' $_.Exception.Message
            $counts.errors++
            $results.Add([pscustomobject]@{ Id = $r.Item.Id; Name = $r.Item.Name; Status = 'ERROR'; Current = $_.Exception.Message; Target = $r.Item.Target })
        }
    }

    foreach ($sd in $StatDefs) { $StatRuns[$sd.Key].Text = [string]$counts[$sd.Key] }
...
$ui.BtnScan.Add_Click({ Invoke-Scan })

# auto-scan on launch so already-applied items show green immediately
$script:AutoScanned = $false
$window.Add_ContentRendered({
    if (-not $script:AutoScanned) { $script:AutoScanned = $true; Invoke-Scan }
})
